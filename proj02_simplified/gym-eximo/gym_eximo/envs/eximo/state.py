...
class State:
    ...
    def valid_position(pos: tuple) -> bool:
        return pos[0] in range(0, 5) and pos[1] in range(0, 5)

    def in_last_row(self, pos: tuple) -> bool:
        return (self.player == 1 and pos[0] == 0) or (self.player == 2 and pos[0] == 4)
    ...
    def enter_place_mode(self) -> None:
        row = 0 if (self.player == 2) else 4
        available = self.board[row][1:4].count(0)
